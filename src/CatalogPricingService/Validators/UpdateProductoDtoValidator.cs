...
namespace CatalogPricingService.Validators
{
    public class UpdateProductoDtoValidator : AbstractValidator<UpdateProductoDto>
    {
        ...
        public UpdateProductoDtoValidator(CatalogDbContext context)
        {
            _context = context;
            ...
            RuleFor(x => x.CodigoBarras)
                ...
                {
                    if (string.IsNullOrEmpty(codigoBarras)) return true;
                    
                    // Unicidad estricta por tenant, PERO excluyendo el ID del producto que estamos editando
                    bool exists = await _context.Productos
                        .AnyAsync(p => p.TenantId == dto.TenantId && p.CodigoBarras == codigoBarras && p.Id != dto.Id, cancellation);
                    
                    return !exists;
                }).WithMessage("El código de barras ya existe en otro producto de su catálogo.");
        }
    }
}
